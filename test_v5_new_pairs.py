"""
Test V5 Asian Range Sweep edge on 3 UNTESTED pairs: NZDUSD, AUDJPY, NZDJPY.
Reuses V5's own functions from multi_pair_backtest.py — does NOT modify it.
Verdict per pair: PF, WR, trades, P&L. Keep candidates with PF >= 1.4.
"""
import sys
...
# New-pair broker constants (from MT5 live query).
# JPY pairs: point=0.001, jpy=True. USD pair: point=0.00001.
# Spread set to realistic NY-session values (MT5 showed 0 for JPY = market closed).
TEST_CONFIG = {
    ...
    "AUDJPY": {"spread": 18, "point": 0.001,   "contract": 100000.0, "lot_max": 50.0,
               "sl_pts": 80, "min_r": 150, "max_r": 1800, "quality": 6, "cat": "Cross",
               "jpy": True,  "risk_override": None},
    "NZDJPY": {"spread": 20, "point": 0.001,   "contract": 100000.0, "lot_max": 50.0,
               "sl_pts": 80, "min_r": 150, "max_r": 1800, "quality": 6, "cat": "Cross",
               "jpy": True,  "risk_override": None},
}
...
def run_pair(sym, cfg):
    ...
    for d in dates:
        ...
        if not sigs:
            continue
        # best-quality sweep wins (same as V5 daily slot logic)
...
def main():
    if not m.connect_mt5():
        ...
        sys.exit(1)
    print(f"Testing V5 edge on untested pairs | {BACKTEST_DAYS}d | risk {RISK_PCT}%")
    print("=" * 60)
    rows = []
